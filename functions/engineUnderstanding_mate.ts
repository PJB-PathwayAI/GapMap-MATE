 ...
 *   3. Return readiness information (does NOT persist lifecycle state)
 *
 * What this engine does NOT do:
 *   - Persist tos_phase (companionService owns lifecycle transitions)
 *   - Persist operational_picture_confirmed (companionService owns confirmation)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // engineUnderstanding reads existing.operational_picture_confirmed for assessment only.
    const merged = {
    ...
    // Read existing confirmation state for assessment (do NOT write)
    ...
    // ─── Readiness assessment (reported only — NOT persisted) ───
    ...
    // ─── Persist Understanding data only ───
    // tos_phase is NOT in the update payload — companionService owns lifecycle transitions.
    // operational_picture_confirmed is NOT in the update payload — companionService owns confirmation.
    // The SDK must preserve these fields on partial update (verified by Packet 2C-T2).
    ...
    return new Response(JSON.stringify({
      ...
      phase_advanced: false, // engineUnderstanding never advances phase
